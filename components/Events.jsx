import React from "react";
import { ArrowRightIcon, CalendarIcon } from "@heroicons/react/24/outline";
import Image from "next/image";
import db from "@/lib/db";
import Link from "next/link";
async function Events({ items }) {
  // Get the number of items from searchParams (e.g., ?items=5), default to 10 if not provided.
  // const items = items ? items : 3;

  // Fetch events ordered by date in descending order (latest first), limited to numberOfItems.
  let events = [];
  if (items) {
    events = await db.events.findMany({
      orderBy: { date: "desc" },
      take: items,
    });
  } else {
    events = await db.events.findMany({
      orderBy: { date: "desc" },
    });
  }

  // const newsItems = [
  //   {
  //     category: "Event",
  //     date: "March 15, 2024",
  //     title: "Annual Research Conference",
  //     excerpt:
  //       "Join leading researchers in discussing breakthrough innovations in technology and science.",
  //     image: "/ourmission.jpg",
  //     alt: "Auditorium full of attendees at research conference",
  //     link: "/news/research-conference",
  //   },
  //   // Add more items
  //   {
  //     category: "Event",
  //     date: "March 25, 2024",
  //     title: "Alumni Meet",
  //     excerpt:
  //       "Join leading researchers in discussing breakthrough innovations in technology and science.",
  //     image: "/ourmission.jpg",
  //     alt: "Auditorium full of attendees at research conference",
  //     link: "/news/research-conference",
  //   },
  //   {
  //     category: "Event",
  //     date: "March 15, 2024",
  //     title: "Annual Research Conference",
  //     excerpt:
  //       "Join leading researchers in discussing breakthrough innovations in technology and science.",
  //     image: "/ourmission.jpg",
  //     alt: "Auditorium full of attendees at research conference",
  //     link: "/news/research-conference",
  //   },
  // ];
  return (
    <section className="py-16 bg-gradient-to-t from-white/95 via-white/70 to-white/95 z-10 relative">
      <div className="mx-auto px-4 max-w-7xl">
        <div className="text-center mb-20">
          <h2 className="text-5xl font-bold text-black dark:text-amber-50 mb-8 relative">
            Latest News & Events
            <span className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-amber-400 to-red-600 rounded-full"></span>
          </h2>
          <p className="text-xl text-gray-800 dark:text-stone-200 max-w-3xl mx-auto">
            Stay updated with our latest achievements, research breakthroughs,
            and campus events
          </p>
        </div>
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {events.map((event, index) => (
            <article
              key={index}
              className="group relative bg-white rounded-sm transition-all duration-300 overflow-hidden"
            >
              <div className="relative h-60 overflow-hidden border-[#764024]/20">
                <Image
                  src={"/" + event.image}
                  alt={event.image}
                  fill
                  className="object-cover transition-transform duration-500 brightness-90 scale-105 group-hover:scale-100"
                  placeholder="blur"
                  blurDataURL="/placeholder-news.jpg"
                />
                <span
                  className={`absolute top-4 right-4 px-3 py-1 rounded-full text-sm font-medium ${
                    event.category === "Event"
                      ? "bg-[#764024] text-[#f5f5f4]"
                      : "bg-[#422416] text-[#f5f5f4]"
                  }`}
                >
                  {event.category}
                </span>
              </div>

              <div className="p-6 bg-[#a44d3c]">
                <div className="flex items-center text-sm text-[#f5f5f4]/90 mb-3">
                  <CalendarIcon className="w-5 h-5 mr-2 text-[#f5f5f4]/80" />
                  {event.date.toLocaleDateString()}
                </div>
                <h3 className="text-xl font-semibold text-[#f5f5f4] mb-2 hover:text-[#f5f5f4]/80 transition-colors">
                  <a href={"#"} className="relative block">
                    {event.title}
                    <span className="absolute inset-0"></span>
                  </a>
                </h3>
                <p className="text-[#f5f5f4]/80 line-clamp-3 mb-4 text-sm leading-relaxed">
                  {event.description}
                </p>
                <div className="flex items-center text-[#f5f5f4] font-medium hover:text-[#f5f5f4]/80 transition-colors">
                  Read More
                  <ArrowRightIcon className="w-4 h-4 ml-2 transform group-hover:translate-x-1 transition-transform" />
                </div>
              </div>
            </article>
          ))}
        </div>

        <div className="mt-12 text-center">
          <Link
            href={"/events"}
            className="inline-flex items-center px-6 py-3.5 bg-gradient-to-r from-[#a44d3c] to-[#a44d3c] text-[#f5f5f4] font-semibold rounded-full hover:scale-[1.02] transition-transform duration-300 shadow-sm hover:shadow-md"
          >
            View All News & Events
            <ArrowRightIcon className="w-5 h-5 ml-3 -mr-1 transform group-hover:translate-x-2 transition-transform" />
          </Link>
        </div>
      </div>
    </section>
  );
}

export default Events;
